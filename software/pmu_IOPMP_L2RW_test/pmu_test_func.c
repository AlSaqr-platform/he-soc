...
void test_pmu_core_L2_SPM_RW (
              uint32_t program_start_addr,
              uint32_t arr_base,
              uint32_t pmc_status_base_addr,
              uint32_t len,
              uint32_t DEBUG) {
  uint32_t program[] = {  // Change this program array
    0x00000033,    
    0x00000033,  
    0x1c0002b7,  
    0x00428293,  
  ...
  //printf("Line before => write_32b(pmc_status_base_addr, 0);");
  write_32b(pmc_status_base_addr, 0);
  printf("Line after => write_32b(pmc_status_base_addr, 0); \n");
 
 // bool value_match = false;
  // Keep polling L2 SPM and comparing L2 SPM outputs.
  //uint32_t *address = (uint32_t *)0x1c000004; // L2 SPM 2nd Address, Ignored the first one as it is used by the jtag sanity check => do not READ/WRITE that register
  //uint32_t value = *address;
  // printf("before while");
  //while (value != 0x12345678) {
    // This is if you have to read
   //value = *address;
	  // value_match = true;
  //}
  //printf("Value at L2 SPM address: %x\n", value);
  // printf("Test passed! :)");
  ...
  //printf(b[0]);
  //printf("\n");
  //while (b[0]!=0x12345678) {
    //printf(".");
  //}
  printf("Error Count: %d \n", error_count);
  if (b[0]==0x12345678) {
    printf("Value at L2 SPM 0x1C000004 address: %x\n", b[0]);
    printf("Read Value matches with what has been written! :)");
    printf("Test passed! :) \n");
  } else {
    printf("Test Failed! :( \n)");
  }
